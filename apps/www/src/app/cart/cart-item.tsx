...
export const SelectedSeatText = ({
  ...
  const { data: fromStation } = useQuery({
    ...
    queryFn: async () => {
      const res = await get(
        `${env.NEXT_PUBLIC_API_BASE_URI}/api/stations/${seat.fromStationId}`,
      );
  ...
  const { data: toStation } = useQuery({
    ...
    queryFn: async () => {
      const res = await get(
        `${env.NEXT_PUBLIC_API_BASE_URI}/api/stations/${seat.toStationId}`,
      );
      return res.data as Station;
    },
  });
  ...
  return seatFullData && fromStation && toStation ? (
    <div className={cn("flex flex-col items-start", className)}>
      ...
      {seatFullData.departTime ? (
        <p>{dayjs(seatFullData.departTime).format("L HH:mm")}</p>
      ) : null}
      {seatFullData.carriage ? (
        <p>{`${seatFullData.carriage.name}, chỗ ${seatFullData.order}`}</p>
      ) : null}
    </div>
  ) : (
    <SkeletonText />
