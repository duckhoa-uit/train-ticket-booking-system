...
export const TripTimelines = ({ disabled }: { disabled?: boolean }) => {
  ...
  const { data: selectedJourney } = useQuery({
    ...
    queryFn: async () => {
      const res = await get(
        `${env.NEXT_PUBLIC_API_BASE_URI}/api/journeys/${selectedJourneyId}`,
      );
      const journey = res.data as Journey & {
  ...
  const { data: selectedTrain } = useQuery({
    ...
    queryFn: async () => {
      const res = await get(
        `${env.NEXT_PUBLIC_API_BASE_URI}/api/trains/${selectedTrainId}`,
      );
    ...
    if (selectedJourney?.id && selectedTrainSeatTypes.length) {
      setValue(
        ...
        selectedJourney.journeyStations.map((journeyStation, index) => {
          if (
            index + 1 === selectedJourney.journeyStations.length ||
          ...
          const prices = selectedTrainSeatTypes.map((seatType) => ({
            ...
            arrivalStationId:
              selectedJourney.journeyStations[index + 1].station.id,
          }));
    ...
    <div className="p-6">
      ...
      <div className="space-y-2">
        <ol className="border-subtle relative border-s">
          {(selectedJourney?.journeyStations ?? []).map(
            (journeyStation, idx) => (
              <li className="mb-10 ms-4" key={journeyStation.id}>
                ...
                <div className="mt-2 grid grid-cols-2">
                  <div>
                    <Label htmlFor={`timelines.${idx}.arrivalDate`}>
                      Arrival Date
                    </Label>
                    <Controller
                  ...
                  <div>
                    <Label htmlFor={`timelines.${idx}.departDate`}>
                      Depart Date
                    </Label>
                    <Controller
                ...
                {selectedJourney &&
                  idx + 1 < +selectedJourney.journeyStations.length && (
                    <>
                      ...
                      <ol className={cn("mt-3 flex flex-col gap-2")}>
                        {selectedTrainSeatTypes.map((type, index) => (
                          <li
                            ...
                            key={type.id}
                          >
                            <div className="min-w-20 mb-3 text-sm">
                              - {type.name}
                            </div>
                            <Controller
