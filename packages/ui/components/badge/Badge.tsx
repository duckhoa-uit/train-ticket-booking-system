...
export type BadgeProps =
  ...
  | (BadgeBaseProps &
      Omit<React.HTMLAttributes<HTMLButtonElement>, 'onClick'> & { onClick: () => void });

export const Badge = function Badge(props: BadgeProps) {
  const { variant, className, size, startIcon, withDot, children, rounded, ...passThroughProps } =
    props;
  ...
  const Children = () => (
    <>
      {withDot ? <Circle className="h-3 w-3 stroke-[3px]" /> : null}
